...
// function filter (weight)

function filterWeight(arr, weightFrom, weightTo) {
  ...
  let arrNew = arr.filter(item => item.weight <= parseInt(weightTo) && item.weight >= parseInt(weightFrom)); // filter array of products
...
export default filterWeight; // export filter (weight)
